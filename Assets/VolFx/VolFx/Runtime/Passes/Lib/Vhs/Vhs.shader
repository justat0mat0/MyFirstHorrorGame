...
Shader "Hidden/Vol/Vhs" 
{
	SubShader 
	{
		Pass 
		{
			...
			#pragma multi_compile_local _STANDALONE_SCANLINES_ON _
			...
			float4 _InputC;	// _Sl_Count, _Sl_Intensity, _Sl_Move, _Sl_Flickering
			#define _Sl_Count       _InputC.x
			#define _Sl_Intensity   _InputC.y
			#define _Sl_Move        _InputC.z
			#define _Sl_Flickering  _InputC.w
...
#ifdef _STANDALONE_SCANLINES_ON
			//i.uv =	float2(i.uv.x, (i.uv.y + _Sl_Flickering) % 1);
			float y = i.uv.y + (_Sl_Move / max(1.0, _Sl_Count) * 6);
#endif
...
#ifdef _STANDALONE_SCANLINES_ON
				float2 sl = float2(sin(y * _Sl_Count), cos(y * _Sl_Count));
				float3 scan = float3(sl.x, sl.y, sl.x);

				vhs.rgb += vhs.rgb * scan * _Sl_Intensity;
#endif
                // Output to screen
